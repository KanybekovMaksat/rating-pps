...
function UserInfoA() {
  ...
  const handleFreezeSelected = async () => {
    try {
      ...
      await axios.put(`http://api.pps.makalabox.com/api/admin/${selectedStage}/freeze`, requestData);
      location.reload();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleActiveSelected = async () => {
    try {
      const idBag = selectedItems.map(itemId => ({ id: itemId }));
      const requestData = { "idBag": idBag };
      await axios.put(`http://api.pps.makalabox.com/api/admin/${selectedStage}/active`, requestData);
      location.reload();
    } catch (error) {
      console.log(error);
    }
